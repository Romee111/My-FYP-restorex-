...
const getReviewsForProduct = catchAsyncError(async (req, res, next) => {
  const { productId } = req.params; // Get the productId from the request params

  // Fetch reviews related to this productId
  const reviews = await reviewModel.find({ productId }).populate('userId', 'name -_id');

  if (!reviews || reviews.length === 0) {
    return next(new AppError("No reviews found for this product", 404));
  }

  // Return reviews for the specified product
